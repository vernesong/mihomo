...
type RawMitm struct {
	...
	CAP12               string   `yaml:"ca-p12" json:"ca-p12"`
}
...
func ParseRawConfig(rawCfg *RawConfig) (*Config, error) {
	...
	mitmConfig, err := parseMitm(rawCfg.Mitm)
	if err != nil {
		return nil, err
	}
...
func parseMitm(raw *RawMitm) (*Mitm, error) {
	...
	prefixes := make([]netip.Prefix, 0, len(raw.ClientSourceAddress))
	for index, value := range raw.ClientSourceAddress {
		prefix, err := netip.ParsePrefix(value)
		if err != nil {
			return nil, fmt.Errorf("mitm.client-source-address[%d]: %w", index, err)
		}
		prefixes = append(prefixes, prefix)
	}

	caP12, err := loadMitmCAP12(raw.CAP12)
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("parse mitm config: %w", err)
	}
	return mitmConfig, nil
}

func loadMitmCAP12(value string) ([]byte, error) {
	if value == "" {
		return nil, fmt.Errorf("mitm.ca-p12 is required")
	}

	resolvedPath := C.Path.Resolve(value)
	if len(value) <= 4096 && !strings.ContainsAny(value, "\r\n") {
		if info, err := os.Stat(resolvedPath); err == nil && !info.IsDir() {
			if !C.Path.IsSafePath(resolvedPath) {
				return nil, C.Path.ErrNotSafePath(resolvedPath)
			}
			data, err := os.ReadFile(resolvedPath)
			if err != nil {
				return nil, fmt.Errorf("read mitm.ca-p12: %w", err)
			}
			return data, nil
		}
	}
	...
	data, err := os.ReadFile(resolvedPath)
	if err != nil {
		return nil, fmt.Errorf("mitm.ca-p12 must be a readable path or base64 data: %w", err)
	}
	return data, nil
}
